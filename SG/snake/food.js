...
class Food extends THREE.Mesh {
  ...
  generateFood(snakeCubes, speedFood, decreaseFood, lessIntesityFood, moreIntensityFood){
    while(true){
      ...
      for(var i=0;i<snakeCubes.length;++i){
        if((snakeCubes[i].position.x == xFood && snakeCubes[i].position.y == yFood) | 
          ...
          (xFood == lessIntesityFood.position.x && yFood == lessIntesityFood.position.y) |

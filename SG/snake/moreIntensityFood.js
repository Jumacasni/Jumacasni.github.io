...
class MoreIntensityFood extends THREE.Mesh {
  ...
  generateFood(snakeCubes, food, speedFood, decreaseFood, lessIntensityFood){
    while(true){
      ...
      for(var i=0;i<snakeCubes.length;++i){
        if((snakeCubes[i].position.x == xFood && snakeCubes[i].position.y == yFood) | 
          ...
          (xFood == lessIntensityFood.position.x && yFood == lessIntensityFood.position.y)){
